...
async function fetchData() {
  try {
    ...
    const { isFetching, error, data } = await useFetch(url, {
      refetch: true // при изменении url будут загружаться новые данные
    }).json()

    isProgress.value = isFetching.value
...
<template>
  <div class="relative overflow-y-auto shadow-md">
    <h1 class="font-bold text-3xl text-center my-2">История</h1>

    <div v-if="isProgress" class="transition duration-500 ease-in-out flex items-center justify-center" role="status">
    ...
    <div v-if="!isProgress">
      <table class="w-full text-sm text-left rtl:text-right text-gray-400">
        <tbody>
        <tr :key="item.id" v-for="item in userInfos.value" class="border-b bg-gray-800 border-gray-700">
          <th scope="row"
            ...
            <div class="ps-3">
              ...
              <div class="font-normal text-gray-500">{{ item.address.street }}</div>
              <div class="font-normal text-gray-500">{{ item.address.suite }}</div>
              <div class="font-normal text-gray-500">{{ item.address.city }}</div>
              <div class="font-normal text-gray-500">{{ item.address.zipcode }}</div>
            </div>
          </th>
          <td class="px-6 py-4">
            <!-- Modal toggle -->
